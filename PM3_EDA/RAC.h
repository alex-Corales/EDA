...
#define Mrac 149 //p= 0.77 -> M = N/p -> M = 110/0.77 -> M = 142.85 -> M = 149.
...
int localizarRAC(celda RAC[] ,int dni ,int *pos){
    ...
    */
    ...
    int k = 0;
    int libre = 0;
    while(j != Mrac && RAC[i].estado != '*'){ //Mientras
    ...
    if(j == Mrac) return 0; //El elemento no se encuentra en la lista, estan todas las celdas llenas
